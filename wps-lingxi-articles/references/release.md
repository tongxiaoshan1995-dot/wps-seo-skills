...
- 版本仓库：`https://github.com/tongxiaoshan1995-dot/wps-seo-skills`（分支 `main`）
...
| 角色 | 位置 | 说明 |
|---|---|---|
| **维护者开发副本** | 本地工作区 `wps-lingxi-articles/`（已 `git init`） | 改技能在这里，改完 push 到 GitHub |
| **GitHub 版本仓库** | 远端 `main` 分支 | 唯一版本源，发布即 push |
| **维护者灵犀已安装副本** | `user_skills/wps-lingxi-articles/` | 灵犀实际调用的那份，改完同步/重装 |
...
1. 打开 **技能 → 我的技能**
2. 找到 `wps-lingxi-articles` → 点击 **... → 上传到云端**（覆盖旧版本）
3. 完成后云端即与 GitHub 一致
...
git clone https://github.com/tongxiaoshan1995-dot/wps-seo-skills.git <本地目录>
# 或使用脚本：
