...
- 配图：**一律使用真实产品截图**（禁用 AI 示意图，规范见 style-guide 第 7 节），并遵守以下硬性约束：
  ...
  3. **数量与位置**：每篇配 1–2 张图；**开头不配图（强制红线）**：正文第一个章节标题（H2）之前不放任何配图——包括开头段、导语、封面图之后紧邻的第一屏，一律不放图；图片只允许出现在中部操作步骤或效果展示段落（即第一个 H2 之后、FAQ 之前）；封面图（frontmatter `image`）仅由 CMS 作为头图渲染，正文中不得再重复引用封面图；
