...
- **灵犀能力硬性植入（红线）**：无论选题是 WPS 相关还是 AI/通用办公相关，正文都必须植入 WPS 灵犀能力（独立章节或明确段落，不能只文末一句带过）；**WPS 功能类文章（下载安装/教程操作/格式转换/故障解决/模板获取/功能认知/品牌直达等组别）必须在结尾（小结段之前或小结段内）用一整段篇幅植入 WPS 灵犀能力**——结合本文讲的具体 WPS 功能，写清灵犀如何把这件事做得更快（如“手动合并单元格要逐个操作，在 WPS 里让灵犀 AI 表格一句话搞定”），不能只在开头或中部一笔带过；灵犀能力的表述以资源池多维表（WPS资源池）中灵犀/AI 相关素材的官方描述为准，不得杜撰能力名称、入口路径和效果数据；实在无法自然植入的选题，须在写作前向用户说明并确认
...
- 配图：**一律使用真实产品截图**（禁用 AI 示意图，规范见 style-guide 第 7 节），并遵守以下硬性约束：
  ...
  3. **数量与位置**：每篇配 1–2 张图；**开头不配图（强制红线）**：正文第一个章节标题（H2）之前不放任何配图——包括开头段、导语，一律不放图；图片只允许出现在中部操作步骤或效果展示段落（即第一个 H2 之后、FAQ 之前）；**封面图默认不配**：frontmatter 不填 `image` 字段（封面图会渲染在正文最前，视觉上就是“开头配图”，与红线冲突）；仅当用户明确要求配封面时才填，且封面图不得与正文图重复计算配图数；
