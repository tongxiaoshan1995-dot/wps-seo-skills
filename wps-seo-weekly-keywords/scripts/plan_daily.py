...
CAT_MAP = {
    "品牌词": {"品牌与官方入口", "下载安装"},
    "竞品词": {"竞品对比与选型"},
    "通用词": {"内容与社媒词", "协同办公与企业", "其他", "云文档与在线办公"},  # 已按运营要求剔除"价格与会员"题材分类
...
def main():
    ...
    for k in words:
        ...
        if k["关键词"] in JUNK_WORDS or JUNK_RE.search(k["关键词"]):
            continue
        kw = k["关键词"]
        # 运营要求：剔除会员/价格相关词（题材与内容层均不产出）
        if re.search(r"(会员|多少钱|价格表|收费|费用|年费|月费|续费|充值|划不划算|性价比|要不要花钱|值不值得|限免|免费版|免费试用|怎么买|购买|开通会员|值不值|贵不贵|免费|价格)", kw):
            continue
        # 已覆盖剔除
