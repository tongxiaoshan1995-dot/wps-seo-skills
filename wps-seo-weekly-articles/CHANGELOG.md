...
**配图方式改为一律真实截图，禁用 AI 示意图**。
- 新增 `scripts/fetch_images.py`：从资源池素材原文按来源优先级（WPS官方公众号>小绿书>WPS社区>WPS客服中心>WPS学堂）提取真实产品截图，自动过滤头像/徽章/图标/SVG 装饰并处理 wpscdn 防盗链（Referer）；支持直接给 URL 或按关键词从资源池自动找素材；输出 `_image_sources.json` 溯源。
- style-guide 第 7 节配图规范重写：一律真实截图，先 fetch_images.py 提取，抓不到用 browser 截图兑底，删除 AI 示意图；article-template / SKILL 同步。
- 顺带修正 SKILL.md 资源列表中 fetch_pool.py 的来源优先级描述（与脚本一致：公众号>小绿书>社区>客服>学堂）。
